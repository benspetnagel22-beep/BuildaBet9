...
# --- Team logos (partial, add more as needed) ---
TEAM_LOGOS = {
...
def fetch_games(sport):
    league_ids = {"NFL":3,"NBA":12,"MLB":1}
    league = league_ids.get(sport)
    if league is None:
        # For CFB we simulate a few games
        return [
            {"home":"Alabama","away":"LSU","time":"7:00 PM","id":"Alabama_LSU"},
            {"home":"Ohio State","away":"Michigan","time":"8:30 PM","id":"OhioState_Michigan"}
        ]
...
def calculate_props(players):
    ...
    for p in players:
        props.append({"player":p['name'],"prop":"Passing Yards","line":p['pass_yards'],
                      "odds":1.9,"reason":"Based on recent performance","EV":round(p['pass_yards']*0.1,2)})
        props.append({"player":p['name'],"prop":"Rushing Yards","line":p['rush_yards'],
                      "odds":1.85,"reason":"Expected workload","EV":round(p['rush_yards']*0.1,2)})
        props.append({"player":p['name'],"prop":"Receiving Yards","line":p['rec_yards'],
                      "odds":1.85,"reason":"Expected target share","EV":round(p['rec_yards']*0.1,2)})
...
# --- Bets Page ---
    ...
    props_df = calculate_props(players)
